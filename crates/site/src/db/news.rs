...
pub async fn list_recent(pool: &PgPool, limit: i64) -> Result<Vec<NewsPost>, AppError> {
    let rows = sqlx::query_as::<_, (String, String, OffsetDateTime)>(
        "SELECT title, body, posted_at FROM news_posts ORDER BY posted_at DESC LIMIT $1",
    )
    .bind(limit)
    .fetch_all(pool)
    .await?;
    ...
    Ok(rows
        .into_iter()
        .map(|(title, body, posted_at)| NewsPost {
            title,
            body,
            posted_at,
        })
...
pub async fn create(
    ...
    if title.is_empty() {
        return Err(AppError::Validation(
            "A news post needs a title.".to_string(),
        ));
    }
    if body.is_empty() {
        return Err(AppError::Validation(
            "A news post needs a body.".to_string(),
        ));
    }
...
mod tests {
    ...
    async fn creates_and_lists_a_post(pool: PgPool) {
        ...
        create(&pool, "Server is open", "Come and play.", author)
            .await
            .unwrap();
    ...
    async fn lists_newest_first_and_honours_the_limit(pool: PgPool) {
        ...
        for n in 1..=3 {
            create(&pool, &format!("Post {n}"), "body", author)
                .await
                .unwrap();

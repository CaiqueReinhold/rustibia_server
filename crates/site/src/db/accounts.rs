...
pub async fn update_password(
    ...
    sqlx::query("DELETE FROM sessions WHERE account_id = $1 AND token_hash <> $2")
        .bind(account_id)
        .bind(crate::auth::token::hash_token(keep_session_token))
        .execute(&mut *tx)
        .await?;
...
mod tests {
    ...
    async fn rejects_a_duplicate_email_case_insensitively(pool: PgPool) {
        ...
        let err = create_account(&pool, "PLAYER@EXAMPLE.COM", "different")
            .await
            .unwrap_err();
    ...
    async fn finds_an_account_regardless_of_email_case(pool: PgPool) {
        ...
        let found = find_by_email(&pool, "PLAYER@example.COM")
            .await
            .unwrap()
            .unwrap();
    ...
    async fn changing_the_password_revokes_other_sessions_but_keeps_this_one(pool: PgPool) {
        ...
        update_password(&pool, account.id, "a-brand-new-password", &keep.token)
            .await
            .unwrap();

        assert_eq!(
            sessions::account_for_token(&pool, &stolen.token)
                .await
                .unwrap(),
            None,
        ...
        assert_eq!(
            sessions::account_for_token(&pool, &keep.token)
                .await
                .unwrap(),
            Some(account.id),
    ...
    async fn the_new_password_replaces_the_old_one(pool: PgPool) {
        ...
        update_password(&pool, account.id, "a-brand-new-password", "irrelevant")
            .await
            .unwrap();
